...
const TYPE_LABELS: Record<string, string> = {
  ...
  dispatch_failed: "Needs Assignment",
};

function NotificationItem({ n, orgSlug, onNavigate }: { n: LogisticsNotification; orgSlug: string; onNavigate: () => void }) {
  ...
  if (n.related_task_id) {
    return (
      <Link href={orgRoute(orgSlug, `/tasks/${n.related_task_id}`)} onClick={onNavigate} className="block hover:bg-muted/30">
        {content}
      </Link>
    );
  }
    ...
    <>
      ...
      <div className="absolute right-0 top-full mt-2 z-50 w-80 rounded-2xl shadow-2xl border border-border bg-popover overflow-hidden">
        ...
        <div className="max-h-96 overflow-y-auto">
          ...
          {notifications.map((n) => (
            <NotificationItem key={n.id} n={n} orgSlug={orgSlug} onNavigate={onClose} />
          ))}
        </div>
      </div>
    </>
